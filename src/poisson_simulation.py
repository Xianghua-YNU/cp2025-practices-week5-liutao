...
def plot_poisson_pmf(lambda_param=8, max_l=20):
    ...
    pmf = (
        lambda_param**l_values * np.exp(-lambda_param)
    ) / factorial(l_values)
...
def simulate_coin_flips(n_experiments=10000, n_flips=100, p_head=0.08):
    ...
    for i in range(n_experiments):
        coins = np.random.choice(
            [0, 1], n_flips, p=[1-p_head, p_head])  # 抛硬币100次
...
def compare_simulation_theory(n_experiments=10000, lambda_param=8):
    ...
    plt.plot(l_values, pmf, 'r-', label='Theoretical Distribution',
             linewidth=2)

    plt.title(
        f'Poisson Distribution Comparison '
        f'(N={n_experiments}, λ={lambda_param})'
    )
